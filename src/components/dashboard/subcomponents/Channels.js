...
class Channels extends Component {
    ...
    getChannel() {
        axios.get('http://localhost:8080/api/v1/asap/channels?peer=' + this.props.username + '&storage=' + this.props.selectedApp)
    ...
    doCreateChannel = () => {
        ...
        let url = 'http://localhost:8080/api/v1/asap/channel?peer=' + this.props.username + '&app=' + this.props.selectedApp;
        axios.post(url, obj)
            .then(res => {
                res.data === null ?
    ...
    render() {
        ...
        const channelListener = "/received/channel/" + this.props.selectedApp;
        ...
        const channelOnClick = (event, index, appName, channel) => {
            ...
            this.props.setMessageMetaInfo(appName, channel, channelSelected)
        }
...

                            {
                                this.state.channels.length === 0 ?
                                    ...
                                    this.state.channels.map((channel, index) => (
                                        <ListItem selected={selectedIndex === index && deselect === true} className={selectedIndex === index && deselect === true ? classes.setSelected : null} onClick={(event) => { channelOnClick(event, index, this.props.selectedApp, channel) }} button id="channels" >
