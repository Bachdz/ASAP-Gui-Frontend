...
class Connection extends Component {
    ...
    doOpenConnection = () => {
        ...
        axios.post(url).then(res => { this.setState({ openedConnection: res.data, showHostInfo: true, openAddConnection: !this.state.openAddConnection, showAddConnection: !this.state.showAddConnection }) });
    }
    doCancelConnection = (e) => {
        const url = 'http://localhost:8080/api/v1/asap/cancelconnection?port=' + this.state.newPort;
    ...
    render() {
        const { showConnectToHost, openConnectToHost, activateConnectToHost, showAddConnection, openAddConnection, activateAddConnection, alertmsg, alertopen, alerttype, openedConnection, showHostInfo } = this.state;
        ...
        const validateHostName = (e) => {
            console.log("Hostname: " + e.target.value)
        }

        const validateHostSocket = (e) => {
            console.log("HostSocket: " + e.target.value)
        }
            ...
            <div className="connection-menu">
                ...
                {showHostInfo ?
                    <div className="hosted-server">
                        ...
                        <CloseIcon id="close-icon" className="icon" onClick={this.doCancelConnection} />
                    </div>
                    :
                    null
                ...
                <div className="setting">
                    ...
                    {showConnectToHost ?
                      ...
                      {
                                openConnectToHost ?
                                    <RemoveIcon id="removeIcon" className="icon" onClick={this.setState({ openConnectToHost: !this.state.openConnectToHost })} />
                                    :
                                    <AddIcon id="addIcon" className="icon" onClick={this.setState({ openConnectToHost: !this.state.openConnectToHost })} />
                    ...
                    }
                    {
                        openConnectToHost ?
                            <div className="add">
                                <input autoComplete="off" type="text" className="input" placeholder="Enter IP/DNS-Name_remoteHost " onChange={validateHostName} />
                                <input autoComplete="off" type="text" className="input" placeholder="Enter remote port socket " onChange={validateHostSocket} />
                    ...
                    {showAddConnection ?
                      ...
                      {
                                openAddConnection ?
                                    <RemoveIcon id="removeIcon" className="icon" onClick={this.setState({ openAddConnection: !this.state.openAddConnection })} />
                                    :
                                    <AddIcon id="addIcon" className="icon" onClick={this.setState({ openAddConnection: !this.state.openAddConnection })} />
