...
class Connection extends Component {
    ...
    doTerminateConnection = (host, port) => {
        ...
        axios.post(url).then(res => {
            if (res.data === true) {
                for (var i = this.state.connectionAttempt - 1; i >= 0; --i) {
                    if (this.state.connectionAttempt[i].host == host && this.state.connectionAttempt[i].port == port) {
                        this.state.connectionAttempt.splice(i, 1);
                    }
                }
                this.setState({ alertopen: !this.state.alertopen, alertmsg: "Connection successfully canceled", alerttype: "error" });
