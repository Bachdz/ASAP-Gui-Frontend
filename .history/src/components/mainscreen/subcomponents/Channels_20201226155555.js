...
class Channels extends Component {
    ...
    render() {
        const { open, openAddChannel, activateAddChannel, alertmsg, alertopen, alerttype, selectedIndex, deselect, showMainScreen } = this.state;
...

                                {
                                    this.state.channels.length === 0 ?
                                        ...
                                        this.state.channels.map((channel, index) => (
                                            <ListItem selected={selectedIndex === index && deselect === true} className={selectedIndex === index && deselect === true ? classes.setSelected : null} onClick={(event) => handleListItemClick(event, index)} button id="channels" >
                                                <ListItemText classes={{ secondary: classes.listItemTextSecondary }} primary={channel.uri} secondary={channel.recipients.length > 0 ? "Recipients: " + channel.recipients : "Recipients: 0"} onClick={this.props.showMainScreen.bind(this, this.props.appName, channel, this.state.showMainScreen)} />
                                            </ListItem>
